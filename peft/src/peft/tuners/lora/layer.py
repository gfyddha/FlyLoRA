...
class LoraLayer(BaseTunerLayer):
    ...
    adapter_layer_names: tuple[str, ...] = ("lora_A", "lora_B", "lora_topk_bias", "lora_embedding_A", "lora_embedding_B") # modified
    # All names of other parameters that may contain adapter-related parameters
    other_param_names: tuple[str, ...] = ("r", "lora_alpha", "scaling", "lora_dropout", "bias_u", "top_k") # modified
    ...
    def update_layer(
        ...
        bias_u: float = 0.01, # modified
        top_k: int = 8, # modified
    ):
        # collect the kwargs
        kwargs = locals().copy()
        del kwargs["self"]
        ...
        self.lora_topk_bias[adapter_name] = nn.Parameter(torch.zeros(r), requires_grad=False) # this should not be updated by the optimizer # modified
        self.bias_u[adapter_name] = bias_u # modified
        self.top_k[adapter_name] = top_k # modified
    ...
    def reset_lora_parameters(self, adapter_name, init_lora_weights):
        ...
        if adapter_name in self.lora_A.keys():
            ...
            elif init_lora_weights.lower() == "sparse": # modified
                weight = self.lora_A[adapter_name].weight # (r, n)
                p = weight.size(1) // (self.r[adapter_name] // self.top_k[adapter_name])
                mask = torch.zeros_like(weight, dtype=torch.bool)
                for i in range(self.r[adapter_name]):
                    indices = torch.randperm(weight.size(1))[:p]
                    mask[i, indices] = True
                nn.init.normal_(weight, mean=0, std=1 / self.r[adapter_name])
    ...
    def __init__(
        ...
        **kwargs,
        ...
        self.update_layer(
            ...
            use_dora=use_dora,
            lora_bias=lora_bias,
            bias_u=kwargs["lora_config"].bias_u, # modified
            top_k=kwargs["lora_config"].top_k, # modified
        )
    ...
    def forward(self, x: torch.Tensor, *args: Any, **kwargs: Any) -> torch.Tensor:
        ...
        if self.disable_adapters:
            ...
            for active_adapter in self.active_adapters:
                ...
                if active_adapter not in self.lora_variant:  # vanilla LoRA, modified
                    with torch.no_grad():
                    ...
                    ci = torch.bincount(selected_experts.flatten(), minlength=self.r[active_adapter]).float()
